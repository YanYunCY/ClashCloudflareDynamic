...
1. 选择协议和端口并创建本地配置。例如：
   ...
   powershell -ExecutionPolicy Bypass -File .\setup.ps1 -Protocol vmess -Port 443

   # VLESS + WebSocket + TLS，端口 8443
   powershell -ExecutionPolicy Bypass -File .\setup.ps1 -Protocol vless -Port 8443

   # Trojan + WebSocket，端口 2053
   powershell -ExecutionPolicy Bypass -File .\setup.ps1 -Protocol trojan -Port 2053

   # 其他 Mihomo 节点类型：复制自定义模板，可选覆盖其端口
   powershell -ExecutionPolicy Bypass -File .\setup.ps1 -NodeTemplatePath .\node_template.local.json -Port 2096
   ```
...
4. 安装推荐的混合任务：
   ...
   ```powershell
   powershell -ExecutionPolicy Bypass -File .\install_hybrid_5000.ps1
   ```
...
6. 诊断与手动轻扫：
   ...
   ```powershell
   python .\dynamic_selector.py --diagnose
   python .\dynamic_selector.py --quick
   ```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\install_aggressive_5000_30min.ps1
```
...
默认只移除计划任务和通知快捷方式，保留程序、日志及 provider：
